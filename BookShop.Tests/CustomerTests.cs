...
namespace BookShop.Tests
{
    ...
    public class CustomerTests
    {
        ...
        #region Метод GenerateRandomCustomer

        /// <summary>
        /// Проверяет, что GenerateRandomCustomer создаёт клиента
        /// после инициализации DatabaseManager.
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldCreateCustomer_AfterDatabaseInit()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();

            // Act - Генерируем случайного клиента
            var customer = Customer.GenerateRandomCustomer();
        ...
        /// <summary>
        /// Проверяет, что GenerateRandomCustomer создаёт клиента корректного типа.
        /// Клиент должен быть либо по жанру, либо по конкретной книге.
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldCreateValidCustomer()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();
            ...
            // Assert - Проверяем корректность типа
            if (customer.WantsGenreOnly)
            {
                // Клиент по жанру - DesiredGenre должен быть задан
                Assert.NotNull(customer.DesiredGenre);
                Assert.False(string.IsNullOrEmpty(customer.DesiredGenre));
            }
            else
            {
                // Клиент по книге - Title и Author должны быть заданы
                Assert.NotNull(customer.DesiredTitle);
                Assert.NotNull(customer.DesiredAuthor);
            }
        }

        /// <summary>
        /// Проверяет, что GenerateRandomCustomer создаёт разных клиентов
        /// при многократном вызове (стохастический тест).
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldCreateVariedCustomers()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();
            ...
            // Act - Генерируем множество клиентов
            for (int i = 0; i < 50; i++)
            {
                var customer = Customer.GenerateRandomCustomer();
                if (customer.WantsGenreOnly)
                    genres.Add(customer.DesiredGenre);
                else
                    titles.Add(customer.DesiredTitle);
            }

            // Assert - Должно быть разнообразие (хотя бы 2 разных значения)
            Assert.True(genres.Count + titles.Count > 1, "Генерация должна создавать разнообразных клиентов");
        }

        /// <summary>
        /// Проверяет, что клиент по жанру получает жанр из базы данных.
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldUseGenreFromDatabase()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();
            var availableGenres = DatabaseManager.Genres;

            // Act - Генерируем клиента по жанру (несколько попыток)
            for (int i = 0; i < 20; i++)
            {
                var customer = Customer.GenerateRandomCustomer();
                if (customer.WantsGenreOnly)
                {
                    // Assert - Жанр из базы данных
                    Assert.Contains(customer.DesiredGenre, availableGenres);
                    return; // Тест пройден
                }
            }

            // Если не попали на клиента по жанру - тест считается пройденным
            // (вероятность 50/50, так что за 20 попыток точно попадём)
        }

        /// <summary>
        /// Проверяет, что клиент по книге получает данные из базы данных.
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldUseBookFromDatabase()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();
            var availableBooks = DatabaseManager.BookAuthorPairs;

            // Act - Генерируем клиента по книге (несколько попыток)
            for (int i = 0; i < 20; i++)
            {
                var customer = Customer.GenerateRandomCustomer();
                if (!customer.WantsGenreOnly)
                {
                    // Assert - Книга из базы данных
                    Assert.Contains(customer.DesiredTitle, availableBooks.Select(p => p.Title));
                    return; // Тест пройден
                }
            }

            // Если не попали на клиента по книге - тест считается пройденным
        }
        ...
        /// <summary>
        /// Проверяет, что генерация множества клиентов работает быстро.
        /// Это тест производительности.
        /// </summary>
        [Fact]
        public void GenerateRandomCustomer_ShouldBeFast()
        {
            // Arrange - Инициализируем базу данных
            DatabaseManager.Initialize();
            var stopwatch = System.Diagnostics.Stopwatch.StartNew();

            // Act - Генерируем 1000 клиентов
            ...
            // Assert - 1000 клиентов должны сгенерироваться менее чем за 1 секунду
            Assert.True(stopwatch.ElapsedMilliseconds < 1000,
                $"Генерация 1000 клиентов заняла {stopwatch.ElapsedMilliseconds} мс");
        }
